...
def build_2D_histgram_classifier(gender_data, height_bins, handspan_bins, min_height, max_height, min_handspan, max_handspan):
    ...
    histogram_data = np.zeros((height_bins, height_bins), dtype='int32')

    height_indices = (np.round(((height_bins - 1) * (np.array(gender_data[:, 1], dtype=float) -
                                                     min_height) / (max_height - min_height)))).astype('int32')

    handspan_indices = (np.round(((handspan_bins - 1) * (np.array(gender_data[:, 2], dtype=float) -
...
def get_probability_by_histogram(height, handspan, histo_female, histo_male, height_bins, handspan_bins, min_height, max_height, min_handspan, max_handspan):

    # get bins index in height and handspan
    height_bins_index = (np.round((height_bins - 1) * (height-min_height) / (max_height - min_height))).astype('int32')
    handspan_bins_index = (np.round((handspan_bins - 1) * (handspan-min_handspan) / (max_handspan - min_handspan))).astype('int32')
...
def draw_3d_histgram_2d_data(female_data, male_data, height_bins, min_height, min_handspan):
...
def write_querysheet_excel(excel_file, query, height_array_female, handspan_array_female, height_array_male, handspan_array_male):
...
def write_histogram_by_bayesian_excel(excel_file, sheet_name, height_array, handspan_array, min_height, max_height, min_handspan, max_handspan):

     # Histogram must be square, use height_bins=22 instead of using handspan_bins
    # histogram_data = np.zeros((len(height_array), len(handspan_array)), dtype='int32')
    bins = int(max_height-min_height+1)
    ...
    constant_height = 1
    constant_handspan = 0.5

    for i in range(bins):
        for j in range(bins):
            query_v = np.array([min_height+(i*constant_height), min_handspan+(j*constant_handspan)])
            worksheet.cell(row=7+i, column=2+j, value=get_count_by_bayesian(query_v, height_array, handspan_array).item())
...
handspan_bins = int((max_handspan - min_handspan) * 2 + 1)
...
histo_female = build_2D_histgram_classifier(female_data, height_bins, handspan_bins, min_height, max_height, min_handspan, max_handspan)
histo_male = build_2D_histgram_classifier(male_data, height_bins, handspan_bins, min_height, max_height, min_handspan, max_handspan)

# write female histogram data to Excel. (The histogram must be square, only height_bins is used.)
write_histogram_excel(excel_file, 'Female Histogram', histo_female, height_bins, min_height, max_height, min_handspan, max_handspan)
write_histogram_excel(excel_file, 'Male Histogram', histo_male, height_bins, min_height, max_height, min_handspan, max_handspan)
...
write_querysheet_excel(excel_file, query, female_data[:, 1], female_data[:, 2], male_data[:, 1], male_data[:, 2])

# write 'Reconstructed Female Histogram' sheet in Excel sheet
write_histogram_by_bayesian_excel(excel_file, 'Reconstructed Female Histogram', female_data[:, 1], female_data[:, 2],
                                  min_height, max_height, min_handspan, max_handspan)

# write 'Reconstructed Male Histogram' sheet in Excel sheet
write_histogram_by_bayesian_excel(excel_file, 'Reconstructed Male Histogram', male_data[:, 1], male_data[:, 2],
                                  min_height, max_height, min_handspan, max_handspan)
